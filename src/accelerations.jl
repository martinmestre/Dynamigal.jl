...
"""Acceleration generated by a MacroParticleSystem"""
function acceleration(mps::MacroParticleSystem, x::AbstractArray{L}, t::T=0.0) where {L<:Real, T<:Real}
    sum_acc = zeros(L, 3)
    for mp ∈ mps
...
function complement(mps::MacroParticleSystem, x::AbstractArray{L}, i::I) where {L<:Real, I<:Integer}
    n = length(mps)
...
function acceleration_c!(mps::MacroParticleSystem, x::AbstractArray{L}, t::T=0.0) where {L<:Real, T<:Real}
...
function acceleration!(mps::MacroParticleSystem, x::AbstractVector{L}, t::T=0.0) where {L<:Real, T<:Real}
    ...
    acc  = mps.accelerations
    @inbounds for i ∈ 1:n
        y = SVector{3}(x[3i-2], x[3i-1], x[3i])
        acc_sum = zero(SVector{3, Float64})
        for j ∈ 1:n
            i == j && continue
...
# test acceleration! vs acceleration_c!
