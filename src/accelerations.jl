...
"""
function drag(p::P, x::AbstractArray{L}, v::AbstractArray{L}, t::T, 𝕗::ChandrasekharFrictionConfig) where {P<:AbstractPotential, L<:Real, T<:Real}
    @unpack lnΛ, mₚ, σₕ = 𝕗
    ν = sqrt(dot(v,v))
    χ = ν /(σₕ√2)
...
drag force.
The algorithm for the drag is dispatched according to the 𝕗 config struct.
⚠ Warning: here x and v are difference vectors between the test position and the potential origin.
"""
function acceleration(p::P, x::AbstractArray{L}, t::T, v::AbstractArray{L}, 𝕗::F) where {P<:AbstractPotential, L<:Real, T<:Real, F<:AbstractFrictionConfig}
    return acceleration(p, x, t) + drag(p, x, v, t, 𝕗)
end
...
function acceleration(::FrictionIncludedTrait, system::LargeCloudMW, u::AbstractVector{L}, t::T=0.0) where {L<:Real, T<:Real}
    ...
    acc_at_cloud = acceleration(mw.pot, Δx, t, Δv)
    acc_at_mw = acceleration(cloud.pot, -Δx, t)
    return SVector{6,L}(acc_at_mw[1], acc_at_mw[2], acc_at_mw[3],
             acc_at_cloud[1], acc_at_cloud[2], acc_at_cloud[3])
end
