...
"""
@with_kw struct GalaFriction{M<:Real, L<:Real, V<:Real} <:AbstractFriction
    mₚ::M # perturber mass
    rₚ::L  # perturbers half-mass radius (=r_hm in B&T 2008)
    σₕ::V # host's mean velocity dispersion
end

GalaFriction(mₚ::M, rₚ::L, σₕ::V) where {M<:Unitful.Mass, L<:Unitful.Length, V<:Unitful.Velocity} =
    GalaFriction(ustrip(uconvert(𝕦.m, mₚ)), ustrip(uconvert(𝕦.l, rₚ)), ustrip(uconvert(𝕦.v, σₕ)) )
...
"""
@with_kw struct TangoForThreeFriction{T<:Real, R<:Real, S<:Real} <:AbstractFriction
    mₚ::R # perturber mass
    lnΛ::T  # Coulomb logarithm
    σₕ::S # host's mean velocity dispersion
end
TangoForThreeFriction(mₚ::R, lnΛ::T, σₕ::S) where {R<:Unitful.Mass, T<:Real, S<:Unitful.Velocity} =
    TangoForThreeFriction(ustrip(uconvert(𝕦.m, mₚ)), lnΛ, ustrip(uconvert(𝕦.v, σₕ)) )
