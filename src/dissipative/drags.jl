...
"""
function drag(fric::GalaFriction, p::P, x::AbstractArray{L}, v::AbstractArray{L}, t::T) where {P<:AbstractPotential, L<:Real, T<:Real}
    ...
    if ν < 𝕗.ϵ_ν
        ...
        b_max = sqrt(dot(x,x))
        lnΛ = log( b_max / max(rₚ, G*mₚ/σₕ^2) )
        χ = ν /(σₕ√2)
        return -4π*G^2*lnΛ*density(p, x, t) * mₚ * ( erf(χ) - (2/√π)*χ*exp(-χ^2) ) * v/ν^3
    end
end
...
But I have consciously ommitted the square root in the lnΛ formula.
"""
...
Here we do not have cut the drage when r<r_min or r>r_max, like in Galpy.
"""
